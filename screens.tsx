...
import { areaStoreIds, storesNear } from './stores';
...
export function ListScreen() {
  ...
  const active = areaStoreIds(origin, maxMiles).filter(storeHasData).slice(0, 3);
  const res = basketTotals(basket.items, active);
          ...
          <View style={{ paddingHorizontal: 18, marginTop: 8 }}>
            ...
            {res.lines.map((ln) => {
              ...
              return (
                ...
                >
                  <View style={{ flex: 1 }}>
                    <Text style={{ color: t.ink, fontSize: 15, fontFamily: sansBold }}>{ln.label}</Text>
                  ...
                  <Pressable onPress={() => basket.remove(ln.cat, ln.id)} hitSlop={10} style={{ padding: 6 }}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 44 }}>
        ...
        <View style={{ paddingHorizontal: 18, gap: 10 }}>
          ...
          >
            ...
            <Text style={{ color: t.brand, fontSize: 15, fontFamily: sansBold }}>New list</Text>
          </Pressable>
        </View>
...
export function AddItemsModal({
  ...
  const [q, setQ] = useState('');
  const query = q.trim().toLowerCase();
  const custom = !basket.isPreset(basket.active.id);
      ...
      <View style={[s.root, { paddingTop: insets.top + 8 }]}>
        ...
        >
          <Text style={[s.h1clean, { flexShrink: 1, marginRight: 12 }]}>
            Add to {basket.active.emoji} {basket.active.label}
          </Text>
          <Pressable
            onPress={onClose}
            hitSlop={10}
